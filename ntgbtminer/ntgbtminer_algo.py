...
def getOnesFromDb(ones):
    m = MongoDb()
    data = m.readNonce1("block_binary3", {"block_1":ones})
    if len(data) > 0:
        return data
    else:
        if ones > 300:
...
def getNonces(block_header_hex):
    c = BitArray(hex=block_header_hex)
    ones = c.bin.count("1")
    data = getOnesFromDb(ones)
    if len(data) > 0:
...
if __name__ == "__main__":
    args = ["sreenath", "1DpMnorqAqtuEuZhD3tNujL83QhD9qEvG4", "0"]
    if len(args) < 3:
        print("Usage: {:s} <coinbase message> <block reward address>".format(sys.argv[0]))
        sys.exit(1)

    standalone_miner(args[0].encode().hex(), args[1], int(args[2]) )
