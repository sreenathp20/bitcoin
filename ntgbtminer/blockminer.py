...
class Process(multiprocessing.Process): 
    ...
    def run(self):
        message = "sreenath"
        address = "1DpMnorqAqtuEuZhD3tNujL83QhD9qEvG4"
        #time.sleep(1) 
        standalone_miner(message.encode().hex(), address, int(self.strt) )
        print("I'm the process with id: {}".format(self.id)) 
  
if __name__ == '__main__': 
    p = []
    RANGE = 100
    idx = 4000000000 / RANGE
    for i in range(RANGE):
        start = i * idx
        proc = Process(i, start)      
        print("proc:",proc) 
        proc.start()  
        proc.join() 
        #print(i)
