...
def rpc_getblocktemplate():
    try:
        data = api_method('getblocktemplate')
        #print(data)
        return data
    except ValueError:
        return {}
    ...
    while extranonce <= 0xffffffff:
        ...
        while nonce <= 0xffffffff:
            ...
            if block_hash < target_hash:
                ...
                api_method('mined_block', {"height": height, "block_hash": block_hash.hex(), "target_hash": target_hash.hex(), "nonce": nonce, "extranonce": extranonce}, 'post')
...
def standalone_miner(coinbase_message, address, debugnonce_start):
    while True:
        ...
        if mined_block:
            ...
            response = api_method('submitblock', {"submission": submission}, 'post')
            #response = rpc_submitblock(submission, block_hash)
            if response['response'] is not None:
                print("Submission Error: {}".format(response['response']))
                break
