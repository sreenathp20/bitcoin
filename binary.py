...
def process(block_header, target):
    s = 'abc'
    s = '000000204cc244cfdef4667ead2316fac3825773f6003a87542a030000000000000000009bb4b621da4f88762ee7f1a13b25b2f82b0e012239bcdde7c712c502b8118c657c77c2655dba0317b8567035'
    #s = '06633f8dff66cdf65679596de22f3e35106ab1c84a29ac8bcc45adda619422f2'
    s = block_header
    #print("s: ", s)
    lb = [bin(ord(c))[2:] for c in s]

    lbs = [(8-len(i))*'0'+i for i in lb]
    ...
    for none_i in range(4294967295):
        ...
        #print("out: ", out)
        if out < target:
            print("Block mined with nonce: ", none_i)
            exit()
...
def process3(block_header, height, hash):
    s = block_header
    bin_block_header = "".join([hexToBin(i) for i in s])
    print("bin_block_header         :", bin_block_header, " ", len(bin_block_header))
    bin_block_header_strip = bin_block_header[:len(bin_block_header)-32]
    nonce = bin_block_header[len(bin_block_header)-32:]
    print("bin_block_header_strip   :", bin_block_header_strip, " ", len(bin_block_header_strip))
    block_1 = bin_block_header_strip.count('1')
    block_0 = bin_block_header_strip.count('0')
    
    nonce_1 = nonce.count('1')
    nonce_0 = nonce.count('0')
    print("block_1:", block_1)
    print("block_0:", block_0)
    print("nonce_1:", nonce_1)
    print("nonce_0:", nonce_0)
    data = {
        "height": height,
        "block_1": block_1,
        "block_0": block_0,
    ...
    m = MongoDb()
    if len(hash[:14]) == hash[:14].count('0'):
            print("True")
            m.insertMany("block_binary3", [data])
    else:
            print("False")
...
height = skip
while hash:
    #hash, block_hedaer, height = bm.checkData(hash)
    m = MongoDb()
    data = m.readWithLimit("blocks", {}, skip)
    for d in data:
        print("height: ", d["height"])
        print("idx: ", height)
        #time.sleep(5)
        hash, block_hedaer, height2, hash1 = bm.checkData3(d)    
        process3(block_hedaer, height2, hash1)
        height += 1
    skip += 100
    #process2(block_hedaer)
    #time.sleep(1)
